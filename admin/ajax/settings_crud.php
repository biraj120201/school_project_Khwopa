...
if (isset($_POST['upd_shutdown'])) {
  $frm_data = ($_POST['upd_shutdown'] == 0) ? 1 : 0; // Toggle logic
...
if (isset($_POST['upd_contacts'])) {
  ...
  $q = "UPDATE `contact_details` SET `address`=?,`gmap`=?,`pn1`=?,`pn2`=?,`email`=?,`tw`=?,`fb`=?,`insta`=?,`threads`=?,`iframe`=? WHERE `sr_no`=?";
  $values = [$frm_data['address'], $frm_data['gmap'], $frm_data['pn1'], $frm_data['pn2'], $frm_data['email'], $frm_data['tw'], $frm_data['fb'], $frm_data['insta'], $frm_data['threads'], $frm_data['iframe'], 1];
